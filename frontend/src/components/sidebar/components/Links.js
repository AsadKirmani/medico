...
export function SidebarLinks(props) {
  ...
  const activeRoute = (route) => {
    const routePaths = [route.path, ...(route.activePaths || [])]
      .filter(Boolean)
      .map((path) => path.toLowerCase());

    return routePaths.some((routePath) =>
      location.pathname.toLowerCase().includes(routePath)
    );
  };
                  ...
                  <Flex align='center' justifyContent='space-between' w='100%'>
                    <HStack
                      ...
                      <Flex
                        ...
                        <Box
                          color={activeRoute(route) ? activeIcon : inactiveColor}
                          me='12px'
                          mt='6px'>
                          {route.icon}
                        </Box>
                        <Text
                          me='auto'
                          color={activeRoute(route) ? activeColor : "secondaryGray.600"}
                          fontWeight='500'
                          fontSize='md'>
                          {route.name}
                        </Text>
